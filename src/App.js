import React, {useState,useEffect} from 'react';
import api from './api';
import CategoryView from './Components/Category/CategoryView';
import SubCategoryView from './Components/SubCategory/SubCategoryView';
import AuthorView from './Components/Author/AuthorView';
import RolView from './Components/Rol/RolView';
import PhysicalBook from './Components/PhysicalBook/PhysicalBookView';
import MainView from './Components/Main/MainView';
import 'bootstrap/dist/css/bootstrap.min.css';
import '/Users/aleja/OneDrive/Documents/2023-2/Software II/FrontendProyecto/Frontend/libramanage/src/index.css';


const App = () => {

  return (
    <div className='letra-Pagina'>
      <nav className="fondo-navLibreria navbar navbar-expand-lg ">
        <div className="container-fluid">
         <a className='navbar-brand titulo-empresa' href='#'>
          <img src="data:image/png;base64,"
            class="me-2 mr-2"
            height="65"
            alt="Libramanage Logo"
            loading="lazy"
        ></img>
          <strong>LibraManage</strong>
          </a>
        </div>  
        <div class="d-flex align-items-center titulo-empresa">
          <div class="dropdown mx-2">
            <button class="btn outlineNav black dropdown-toggle" type="button" id="gestionLibreria" data-bs-toggle="dropdown" aria-expanded="false">
              Gestion Libreria
            </button>
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              <li><a class="dropdown-item" href="#">Informes</a></li>
              <li><a class="dropdown-item" href="#">Multas</a></li>
            </ul>
          </div>
          <div class="dropdown mx-2">
            <button class="btn outlineNav black dropdown-toggle" type="button" id="gestionUsuarios" data-bs-toggle="dropdown" aria-expanded="false">
              Gestion Usuarios
            </button>
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              <li><a class="dropdown-item" href="#">Usuarios</a></li>
              <li><a class="dropdown-item" href="#">Roles</a></li>
            </ul>
          </div>
          <div class="dropdown mx-2">
            <button class="btn outlineNav black dropdown-toggle" type="button" id="gestionLibros" data-bs-toggle="dropdown" aria-expanded="false">
              Gestion Libros
            </button>
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              <li><a class="dropdown-item" href="#">Libros Físicos</a></li>
              <li><a class="dropdown-item" href="#">Libros Digitales</a></li>
            </ul>
          </div>
          <div class="dropdown mx-2">
            <button class="btn outlineNav black dropdown-toggle" type="button" id="gestionEntidades" data-bs-toggle="dropdown" aria-expanded="false">
              Gestion Entidades
            </button>
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
              <li><a class="dropdown-item" href="#">Categorias</a></li>
              <li><a class="dropdown-item" href="#">Subcategorias</a></li>
              <li><a class="dropdown-item" href="#">Autores</a></li>
            </ul>
          </div>
          <button data-mdb-ripple-init type="button" class="btn outlineNav black mt-2 mx-2">
            Buscar Libro
          </button>

          <a data-mdb-ripple-init type="button" class="nombre-usuario mt-2 mx-2">
            nombreUsuario
          </a>
          
          <button data-mdb-ripple-init type="button" class="btn outlineNav black mt-2 mx-2">
            Iniciar Sesión
          </button>
          <button data-mdb-ripple-init type="button" class="btn outlineNav black mt-2 mx-2">
            Registrarse
          </button>
        </div>
      </nav>

      <body>
      <MainView />
      </body>
    </div>
  );

}

export default App;
