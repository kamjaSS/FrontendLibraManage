...
function PopupDeleteAuthor({autorDel, onDelete}) {
  const [show, setShow] = useState(false);

  const handleClose = () => setShow(false);
  const handleShow = () => setShow(true);

  const handleDeleteAuthor = async (event) => {
    event.preventDefault();
    console.log(autorDel);
    await api.delete(`/delete_authors/${autorDel.id}`);
    onDelete();
    handleClose();
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Modal.Body>
          Desea eliminar el autor <strong>{autorDel.nombre}</strong>?
        </Modal.Body>
        <Modal.Footer>
...
export default PopupDeleteAuthor;