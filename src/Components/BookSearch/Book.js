...
export const Book = ({ book, autor, categoria, subcategoria}) => {
  ...
  const buyBook = async() => {
    try {
      ...
      console.log(buyForm);
      const response = await api.post('/new_buyBook/', buyForm ,
        {
  ...
  const loanBook = async () => {
    try {
      if (selectedDate === '') {
        ...
        const response = await api.post('/new_loan/', loanForm,
          {
            headers: { 'Authorization': `Bearer ${fetchToken()}` }
          });
        if (await response.status === 200) {
          ...
          {book.precio != null ? (
            <div>
              ...
              <button className='btn btn-outline-success' onClick={buyBook}>
                ...
                <Link to='/login' className='btn btn-outline-primary'>Login para comprar</Link>
              )
              }
            </div>
          ) : (
            <div>
              ...
              {fetchToken() ? (
                <button className='btn btn-outline-warning' onClick={loanBook} disabled={available === 0 || available === null ||available === 1}>
                  ...
                  <Link to='/login' className='btn btn-outline-primary'>Login para reservar</Link>
                )}
            </div>
          )}     
        </div>
